...
bool dfs(int x, int y, int ax, int ay, int cx, int cy, vector<vector<bool>> &visited, int n)
{
  if (x == cx && y == cy)
    return true;
  ...
  for (int i = 0; i < 8; i++)
  {
    ...
    {
      if (dfs(nx, ny, ax, ay, cx, cy, visited, n))
        return true;
    }
  }
...
bool canKingEscape(int n, int ax, int ay, int bx, int by, int cx, int cy)
{
  ...
  return dfs(bx, by, ax, ay, cx, cy, visited, n);
}
